...
exports.pay_an_application = function (req, res) {
  ...
  payAnApplicationValidator(applicationId, body).then(
    ...
    if (error.errors.length() == 1 && error.errors.contains('Ivalid role for the actor. Must be EXPLORER')) {
      res.status(403);
    } else {
      res.status(400);
    }
...
exports.change_an_application_status = function (req, res) {
  const applicationId = req.params.app_id;
  const body = req.body;
  changeAnApplicationStatusValidator(applicationId, body).then(
    ...
    if (error.errors.length == 1 && (error.errors.contains("The actor does not manage the application, because he does not manage the trip") || error.errors.contains("The actor did not make the application"))) {
      res.status(403);
    } else {
      res.status(400);
    }
...
var payAnApplicationValidator = function (applicationId, body) {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    if (Object.keys(body).length != 2 || !body.actorId || !body.paid) {
      ...
      await Actor.findById(body.actorId, function (error, actor) {
        if (error) {
          logger.error(`Error searching an actor with id ${body.actorId}.`);
          errors.push('Error searching an actor with id' + body.actorId);
          ...
          Application.findById(applicationId, function (error, application) {
            if (error) {
            ...
            else if (application.explorerId != body.actorId) {
    ...
    //Necesario el async - await anterior para que no se ejecute antes de tiempo 
    if (errors.length > 0) {
...
//body: {status: "REJECTED"||"DUE"||"CANCELLED", statusReason: "abc", actorId: "123abc"}
var changeAnApplicationStatusValidator = function (applicationId, body) {
  return new Promise(async function (resolve, reject) {
    var errors = [];
      ...
      await Application.findById(applicationId, function (error, application) {
        if (error) {
          ...
          Actor.findById(actorId, function (error, actor) {
            if (error) {
              logger.error(`Error when searching an actor with id ${actorId}`);
              errors.push("Error when searching an actor with id " + actorId);
            } else if (actor.role === 'MANAGER') { //MANAGERS
              if (application.status === 'PENDING') {
                Trip.find({ ticker: application.tripId }, function (error, trip) {
                  if (error) {
                    logger.error(`Error when searching for a trip with id ${application.tripId}`);
                    errors.push("Error when searching for a trip with id " + application.tripId);
                  } else {
                    if (trip.managedBy != actorId) {
                      logger.error(`The actor with id ${actorId}, does not manage the trip with id ${trip.ticker}`);
                      error.push("The actor does not manage the application, because he does not manage the trip");
                    } else if (body.status != "DUE" || body.status != "REJECTED") {
                      logger.error(`Invalid application status ${body.status}, must be DUE or REJECTED`);
                      error.push("Application status must be DUE or REJECTED");
                      ...
                      error.push("If the new application status is REJECTED, a reason must be provided");
                    }
                  }
                });
              } else {
                logger.error("The application state must be PENDING");
                errors.push("The application state must be PENDING");
              }
            } else if (actor.role === 'EXPLORER') { //EXPLORERS
              if (application.explorerId != actorId) {
                logger.error(`The actor with id ${actorId}, did not make the application`);
                errors.push("The actor did not make the application");
              } else if (body.status != 'CANCELLED') {
                logger.error("The explorer can only cancell the application");
                errors.push("The explorer can only cancell the application");
                ...
                errors.push("An explorer only can cancell an application with status PENDING  or ACCEPTED");
              }
            }
          });
        }
